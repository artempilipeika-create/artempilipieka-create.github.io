...
import {MeshFactory} from './module-mesh.mjs';
...
export class PlannerScene{
  constructor(canvas,adapter,onLost){
    ...
    this.scene.add(new THREE.HemisphereLight('#ffffff','#c1c5b9',1.5));
    this.sun=new THREE.DirectionalLight('#fffaf0',2.2);this.sun.position.set(3,7,5);this.sun.castShadow=true;
    ...
    this.selectedBox=new THREE.Box3Helper(new THREE.Box3(),0x346d4a);this.selectedBox.material.depthTest=true;this.selectedBox.renderOrder=20;this.selectedBox.visible=false;this.scene.add(this.selectedBox);
    this.hoverBox=new THREE.Box3Helper(new THREE.Box3(),0x9cb49b);this.hoverBox.material.depthTest=true;this.hoverBox.visible=false;this.scene.add(this.hoverBox);
    ...
    this.resize();this.sync();this.fit('room');
  }
  ...
  disposeTree(group){
    if(!group)return;group.traverse(o=>{if(o.geometry&&o.geometry!==this.factory.geometry)o.geometry.dispose();if(o.userData.ownMaterial)o.material?.dispose();});group.removeFromParent();
  }
  roomMesh(){
    ...
    make(r.width,20,r.depth,0,-12,0,'#e4e4da');this.walls=[];
    for(const sign of[-1,1]){
      const z=make(r.width,r.height,20,0,r.height/2,sign*(r.depth/2+10),'#eeeee6');z.userData.axis='z';z.userData.sign=sign;this.walls.push(z);
      const x=make(20,r.height,r.depth,sign*(r.width/2+10),r.height/2,0,'#e8eae2');x.userData.axis='x';x.userData.sign=sign;this.walls.push(x);
    }
    ...
    const grid=new THREE.LineSegments(geometry,new THREE.LineBasicMaterial({color:0xb6bdae,transparent:true,opacity:.21}));grid.userData.ownMaterial=true;this.roomRoot.add(grid);
  }
  sync(){
    ...
    for(const [id,entry]of this.entries)if(!ids.has(id)){entry.group.removeFromParent();this.entries.delete(id);}
    for(const it of this.adapter.items){
      const signature=this.factory.signature(it);let entry=this.entries.get(it.item_id);
      if(!entry||entry.signature!==signature){if(entry)entry.group.removeFromParent();entry={group:this.factory.build(it),signature};this.entries.set(it.item_id,entry);this.root.add(entry.group);}
    ...
    if(this.dressingKey!==dressing){this.dressingKey=dressing;this.dressing?.removeFromParent();this.dressing=this.factory.dress(this.adapter.items);this.scene.add(this.dressing);}
    if(this.dressing)this.dressing.visible=this.layer==='all'||this.layer==='base';
  ...
  hover(id){const entry=this.entries.get(id);this.hoverBox.visible=Boolean(entry?.group.visible&&id!==this.adapter.selected?.item_id&&!this.preview);if(this.hoverBox.visible)this.hoverBox.box.setFromObject(entry.group).expandByScalar(.003);this.invalidate();}
  placeLabel(){
  ...
  fit(which='kitchen'){
    ...
    if(this.mode==='3d'){
      ...
      const dir=new THREE.Vector3(.65,.42,1).normalize();
      ...
      for(const x of [box.min.x,box.max.x])for(const y of [box.min.y,box.max.y])for(const z of [box.min.z,box.max.z]){
        ...
        distance=Math.max(distance,towardsCamera+Math.abs(delta.dot(right))/(tanH*.84),towardsCamera+Math.abs(delta.dot(up))/(tanV*.78));
      }
      ...
      this.orthoSpan=Math.max(.35,vertical/2,horizontal/2/(this.width/this.height))*1.35;this.camera.zoom=1;this.resize();
    }
  ...
  showPreview(it,error,guides=[]){
    if(!this.preview||this.preview.userData.itemId!==it.item_id||this.previewSignature!==this.factory.signature(it)){
      this.preview?.removeFromParent();this.preview=this.factory.build(it,true);this.previewSignature=this.factory.signature(it);this.scene.add(this.preview);
    }
    ...
    const r=this.adapter.room;
    for(const g of guides){let a,b;if(g.axis==='x'){a=new THREE.Vector3(g.value*S,.004,-r.depth*S/2);b=new THREE.Vector3(g.value*S,.004,r.depth*S/2);}else if(g.axis==='z'){a=new THREE.Vector3(-r.width*S/2,.004,g.value*S);b=new THREE.Vector3(r.width*S/2,.004,g.value*S);}else{a=new THREE.Vector3(-r.width*S/2,g.value*S,it.z*S);b=new THREE.Vector3(r.width*S/2,g.value*S,it.z*S);}
      const geometry=new THREE.BufferGeometry().setFromPoints([a,b]);const line=new THREE.Line(geometry,new THREE.LineBasicMaterial({color:error?0xbd794a:0x50875a,depthTest:false}));line.userData.ownMaterial=true;line.renderOrder=30;this.guides.add(line);
    }
    this.invalidate();
  }
  clearPreview(){this.preview?.removeFromParent();this.preview=null;this.disposeTree(this.guides);this.guides=new THREE.Group();this.scene.add(this.guides);this.sync();}
