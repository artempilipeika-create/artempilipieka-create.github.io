...
function blankDetail(){return {detail_id:crypto.randomUUID(),name:'',comments:'',length:'',width:'',qty:1,variant_id:null,materialLabel:'Материал не выбран',supply_source:'company',rotation:false,grain:'unknown',route:'solid',glue_backing:null,packaging:false,edges:{}};}
...
function backingFromRevision(b){if(!b)return null;return {draft_row_id:b.draft_row_id||null,resolution_reason:b.resolution_reason||null,variant_id:b.material?.variant_id||null,materialLabel:materialLabel(b.material),custom_customer:b.customer_material_key?{key:b.customer_material_key,article:b.material.article||'',manufacturer:b.material.manufacturer||'',name:b.material.name,thickness:b.material.thickness,length:b.material.length,width:b.material.width,reason:b.material.reason}:null,supply_source:b.supply_source||'company',provided_sheets:b.provided_sheets,customer_reason:b.customer_reason};}
function fromRevision(d){return {...blankDetail(),detail_id:d.detail_id,name:d.name||'',comments:d.comments||'',...(d.draft_row_id?{draft_row_id:d.draft_row_id,resolution_reason:d.resolution_reason||'Подтверждение деталировки в таблице'}:{}),length:d.length,width:d.width,qty:d.qty,variant_id:d.material?.variant_id||null,materialLabel:materialLabel(d.material),custom_customer:d.customer_material_key?{key:d.customer_material_key,article:d.material.article||'',manufacturer:d.material.manufacturer||'',name:d.material.name,thickness:d.material.thickness,length:d.material.length,width:d.material.width,reason:d.material.reason}:null,supply_source:d.supply_source,provided_sheets:d.provided_sheets,customer_reason:d.customer_reason,rotation:d.rotation,grain:d.grain,route:d.route,glue_backing:backingFromRevision(d.glue_backing),packaging:d.packaging,edges:Object.fromEntries(sideNames.map(s=>[s,{edge_id:d.edges[s]?.edge?.edge_id||null,supply_source:d.edges[s]?.supply_source||'company',unresolved:d.edges[s]?.state==='unresolved',selection_mode:d.edges[s]?.selection_mode||'manual'}]))};}
function fromImported(r){
 ...
 const glueText=[v.name,v.comments,v.material,v.article,v.row_type].filter(Boolean).join(' ');
 return {...blankDetail(),detail_id:r.draft_row_id,draft_row_id:r.draft_row_id,resolution_reason:'Подтверждение импортированной строки в таблице',name:String(v.name||''),comments:String(v.comments||''),length:v.length??'',width:v.width??'',qty:v.qty??'',variant_id:m?.variant_id||null,materialLabel:materialLabel(m),grain:grainNames[v.texture]?v.texture:'unknown',rotation:v.rotation===true,route:MFEntry.glueHint({_sourceGlueText:glueText})?'glued_18_18':'solid',...customer,
  edges:Object.fromEntries(sideNames.map(side=>{const e=s.edges?.[side]||{};return [side,{edge_id:e.edge_id||null,supply_source:'company',unresolved:e.mark!=='none'&&!e.confirmed&&e.mode!=='manual_override',selection_mode:e.selection_mode||'manual'}];})),_source:'Excel: '+s.sheet+', строка '+s.row,_sourceRow:s.row,_sourcePosition:v.position??'',_sourceGlueText:glueText,_errors:(s.errors||[]).filter(e=>!sideNames.includes(e.field)||!s.edges?.[e.field]?.confirmed),_materialSource:String(v.article||v.material||'Материал не указан')};
}
...
function glueBackingLabel(b){if(!b)return '';const m=b.variant_id?catalogueMaterials.find(x=>x.variant_id===b.variant_id):null;return m?materialLabel(m):(b.custom_customer?materialLabel(b.custom_customer)+' · материал клиента':b.materialLabel||'Материал подклейки');}
...
function addGroupDetail(source){const r=blankDetail();if(source){copyGroupMaterial(r,source);r.grain=source.grain;r.rotation=source.rotation;r.route=source.route;if(source.glue_backing){r.glue_backing=structuredClone(source.glue_backing);delete r.glue_backing.draft_row_id;delete r.glue_backing.resolution_reason;}}manualRows.push(r);dirty=true;renderManual();document.querySelector('[data-detail-id="'+r.detail_id+'"] input')?.focus();}
function renderManual(){
 ...
 for(const [gi,group]of groups.entries()){
  ...
  if(r.route==='glued_18_18')routeBox.append(node('small',r.glue_backing?'Подклейка: '+glueBackingLabel(r.glue_backing):'Подклейка: тот же материал','muted'));
...
async function editRow(index,source){
 ...
 const backing=node('details');backing.append(node('summary','Подклейка / второй слой 18 мм'));const backingStatus=node('p',r.glue_backing?'Выбрано: '+glueBackingLabel(r.glue_backing):'По умолчанию используется тот же материал, что и основной.','selected-material');backing.append(backingStatus);
 const sameBacking=button('Подклейка тем же материалом',()=>{const link=r.glue_backing?.draft_row_id?{draft_row_id:r.glue_backing.draft_row_id,resolution_reason:r.glue_backing.resolution_reason||'Подклейка подтверждена в деталировке'}:{};r.glue_backing=Object.keys(link).length?{...link,variant_id:r.variant_id||null,custom_customer:r.custom_customer?structuredClone(r.custom_customer):null,supply_source:r.supply_source,provided_sheets:r.provided_sheets,customer_reason:r.customer_reason,materialLabel:r.materialLabel}:null;backingStatus.textContent='Подклейка: тот же материал.';},true);
 backing.append(sameBacking,picker('Другой материал подклейки','materials',catalogueRelease,(m,release)=>{const link={draft_row_id:r.glue_backing?.draft_row_id||null,resolution_reason:r.glue_backing?.resolution_reason||null};r.glue_backing={...link,variant_id:m.variant_id,custom_customer:null,supply_source:'company',provided_sheets:null,customer_reason:null,materialLabel:materialLabel(m)};catalogueRelease=catalogueRelease||release;backingStatus.textContent='Подклейка: '+r.glue_backing.materialLabel;}));
 backing.append(button('+ Свой материал подклейки',async()=>{const seed={custom_customer:r.glue_backing?.custom_customer||null,provided_sheets:r.glue_backing?.provided_sheets};const value=await customerMaterialDialog(seed);if(!value)return;const link={draft_row_id:r.glue_backing?.draft_row_id||null,resolution_reason:r.glue_backing?.resolution_reason||null};r.glue_backing={...link,...value};backingStatus.textContent='Подклейка: '+r.glue_backing.materialLabel;},true));
 backing.hidden=route.value!=='glued_18_18';route.onchange=()=>{backing.hidden=route.value!=='glued_18_18';};form.append(backing);
...
async function saveRevision(refresh=true){
 ...
 for(const [i,r]of manualRows.entries()){
  ...
  const {materialLabel,_source,_sourceRow,_sourcePosition,_sourceGlueText,_glueAuto,_glueSourceRows,_errors,_materialSource,_group,...data}=r;
  if(data.glue_backing){const {materialLabel:backingLabel,...cleanBacking}=data.glue_backing;data.glue_backing=cleanBacking;}
