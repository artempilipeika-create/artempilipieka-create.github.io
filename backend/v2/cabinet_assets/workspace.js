...
async function editRow(index,source){
 ...
 const backing=node('details');backing.append(node('summary','Подклейка / второй слой 18 мм'));const backingStatus=node('p',r.glue_backing?.materialLabel?'Выбрано: '+r.glue_backing.materialLabel:'По умолчанию используется тот же материал, что и основной.','selected-material');backing.append(backingStatus);
