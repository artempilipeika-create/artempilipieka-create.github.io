...
def validate_template(t):
    ...
    if set(policy)-{'mode','sheets','header_row'} or policy.get('mode') not in {'explicit','named'}: raise ValueError('Invalid sheet policy')
    ...
    if set(inheritance)-{'enabled','header_marker','service_markers','blank_resets','continuation_marker','confirmed_continuation'}: raise ValueError('Invalid inheritance rules')
    if inheritance.get('enabled') and ('row_type' not in mapping or not inheritance.get('header_marker')): raise ValueError('Explicit material header marker required')
...
def parse(workbook,t,selected):
    ...
    for sheet in workbook['sheets']:
        ...
        if len(sheet['rows'])<header_row: raise ValueError('Header row absent')
        actual={c:sheet['rows'][header_row-1]['cells'].get(c,{}).get('value') for c in t['headers']}
        ...
        for source in sheet['rows']:
            ...
            if n<=header_row: result['reason']='header_or_preamble'
            ...
            elif inheritance.get('enabled') and marker==inheritance['header_marker']:
                # Recognize before dimensional validation. A new block always clears old article.
                inherited={k:raw.get(k) for k in ('article','material','manufacturer','structure','thickness','format_length','format_width')}
