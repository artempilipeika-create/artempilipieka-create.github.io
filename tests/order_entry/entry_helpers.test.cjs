...
test('glue pairing recovers unresolved backing from current catalogue source article',()=>{
...
test('glue backing accepts multiple current variants only when physical identity is identical',()=>{
 const a={draft_row_id:'a',variant_id:'front',materialLabel:'Front 18',supply_source:'company',length:600,width:400,qty:1,route:'solid',edges:{},_sourcePosition:'7',_sourceRow:10,_sourceGlueText:'Гот.дет тол 36 мм',_materialSource:'QA621 PO'};
 const b={draft_row_id:'b',variant_id:null,materialLabel:'Материал не выбран',supply_source:'company',length:600,width:400,qty:1,route:'solid',edges:{},_sourcePosition:'7',_sourceRow:11,_sourceGlueText:'Полка (Копия)',_materialSource:'621 PE'};
 const same=[{variant_id:'back-a',article:'621 PE',name:'A',thickness:18,length:2440,width:1220},{variant_id:'back-b',article:'621 PE',name:'B',thickness:18,length:2440,width:1220}];
 const rows=h.recognizeGlueRows([structuredClone(a),structuredClone(b)],same);assert.equal(rows.length,1);assert.ok(rows[0].glue_backing.variant_id);
 const conflict=[...same,{variant_id:'back-c',article:'621 PE',name:'C',thickness:18,length:2800,width:2070}];
 const unresolved=h.recognizeGlueRows([structuredClone(a),structuredClone(b)],conflict);assert.equal(unresolved.length,2);
});
test('same position and geometry without glue evidence stays as two normal details',()=>{
 ...
 const rows=h.recognizeGlueRows([a,b]);assert.equal(rows.length,2);assert.equal(rows[0].route,'solid');assert.equal(rows[1].route,'solid');
});
